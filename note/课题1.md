...
然后构造顶点的初始向量（包括水平集值和网格内外部属性）用于网格卷积。
...
卷积操作主要是提取网格的特征，主要用来做分类和分割等，如果是曲线生成，可能并不太需要全局特征？只需要局部特征？
...
1. **使用优化的思想，将各种约束都加入能量函数，通过最小化能量函数，进行水平集更新。**
   ...
   损失函数为：$\mathcal{E} = E_{shape} + \lambda_1 E_{sdf} + \lambda_2 E_{topology}$
   $$
