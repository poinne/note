...
第二部分是一个自编码器，用于3d模型的编解码，编码器将3d模型转换为642维嵌入向量，解码器学习将嵌入向量重建为3d模型。
...
作者分别使用设计的sdf loss 和 交叉熵损失函数 来训练网络，使用交并集chamfer来对比两种方式。
...
1. 这几篇论文可以分为两类
2. 每篇论文中水平集是如何在模型中表示的
   - 模型输入是什么？二维图像？网格？体素？
3. 每篇论文中水平集是如何起作用的\如何更新的
4. 
