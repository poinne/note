...
- **提出 PhyCRNet 和 PhyCRNet-s 网络架构**: 这两种网络架构结合了卷积神经网络和循环神经网络的优势，能够有效地解决低维时空参数化的 PDE 问题。
- **硬编码 I/BCs**: 通过特定的填充机制将 I/BCs 硬编码到网络中，提高了边界处的解决方案准确性和泛化能力。
- **无需标签数据**: PhyCRNet 和 PhyCRNet-s 可以在没有标签数据的情况下进行训练，这对于数据驱动的科学计算非常有用。

**与 PINN 的区别**:

- **网络架构**: PINN 使用 FCNN，而 PhyCRNet 使用 ConvLSTM。
- **训练数据**: PINN 通常需要大量的训练数据，而 PhyCRNet 可以在没有标签数据的情况下进行训练。
- **损失函数**: PINN 的损失函数包含 PDE 残差和 I/BC 损失，而 PhyCRNet 的损失函数仅包含 PDE 残差。
- **时间维度处理**: PINN 将时间作为一个额外的维度，而 PhyCRNet 使用 RNN 模块来处理时间维度。

**应用前景**:

- **数据驱动的科学计算**: PhyCRNet 和 PhyCRNet-s 可以用于数据驱动的建模、逆分析和数据同化，尤其是在数据稀缺和噪声较大的情况下。
- **不规则空间域**: 通过结合图神经网络，可以将 PhyCRNet 扩展到不规则空间域。
- **高阶时间演化**: 可以将 PhyCRNet 从前向欧拉格式修改为高阶差分格式，以提高时间演化的准确性。

![image-20241026212530076](https://raw.githubusercontent.com/poinne/md-pic/main/image-20241026212530076.png)
...
- 模型训练
  ...
  模型架构为CNN+RNN，并结合编码器-解码器结构。事先设置好训练时的时间步长度（和数据集生成时的时间步保持一致），CNN编码器用于将当前的状态编码为低维特征【通过卷积操作进行降维和特征处理】，之后通过ConvLSTM根据状态特征和前一步的隐藏特征生成下一步的隐藏特征，再通过解码器得到下一步的状态。
...
分子部分  衡量了解向量变化 和残差变化之间的相似性。这表示了解向量的更新和残差更新之间的关系。

分母部分 表示残差变化的大小。这个项确保了步长的大小与残差的变化幅度成比例，以避免步长过大或过小。
